...
// JUnit 5 test source path under the bundled persistence-hexagonal layout.
// It is the test-side mirror of ContractPathMapper: it returns the same
// package suffix as the production mapper so that the test class lives in
// the same Java package as its subject, only rooted at src/test/java
// instead of src/main/java.
//
// Mapping (bundled persistence-hexagonal layout):
//
...
//
// Interface contracts (input-port, output-port) and persistence-adapter
...
// (input-port, output-port, persistence-adapter). The returned path uses '/'
// separators.
func (TestPathMapper) Map(t model.ContractType, name string) (string, error) {
	...
	case model.ContractAggregate:
		return "domain/" + name + "Test.java", nil
	case model.ContractInputPort, model.ContractOutputPort, model.ContractPersistenceAdapter:
