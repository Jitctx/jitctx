...
// newAuditCmdForUsecaseImplStereotype builds a real cobra audit command wired
// with real infrastructure adapters pointing at the given workDir.
// This is a local copy of the helper from auditIntegration_test.go per Q3 resolution
// (no upstream refactor in this PR).
func newAuditCmdForUsecaseImplStereotype(t *testing.T, workDir, manifestPath string) (*bytes.Buffer, *bytes.Buffer, func(args ...string) error) {
	t.Helper()
...
// TestAuditCmd_Integration_UsecaseImplStereotype_BothPresentNoViolation verifies
// that when a UseCaseImpl class declares both @Service and @RequiredArgsConstructor,
// the audit reports no sintatic violations and does not mention the rule ID.
// Backs PC01US-002 Scenario 1.
func TestAuditCmd_Integration_UsecaseImplStereotype_BothPresentNoViolation(t *testing.T) {
	t.Parallel()

	workDir := t.TempDir()
	copyFixture(t, fixtureDir(t, "pc01us002UsecaseImplStereotype", "projectClean"), workDir)

	manifestPath := filepath.Join(workDir, "project-state.yaml")
	stdout, _, run := newAuditCmdForUsecaseImplStereotype(t, workDir, manifestPath)
	...
	require.NotContains(t, out, "usecase-impl-stereotype",
		"clean project must not mention the rule ID in the report")
}

// TestAuditCmd_Integration_UsecaseImplStereotype_MissingRequiredArgsConstructorReportsEvidence
// verifies that when a UseCaseImpl class is missing @RequiredArgsConstructor,
// the audit reports exactly one violation with evidence naming the missing annotation.
// Backs PC01US-002 Scenario 2.
func TestAuditCmd_Integration_UsecaseImplStereotype_MissingRequiredArgsConstructorReportsEvidence(t *testing.T) {
	t.Parallel()

	workDir := t.TempDir()
	copyFixture(t, fixtureDir(t, "pc01us002UsecaseImplStereotype", "projectMissing"), workDir)
	...
	out := stdout.String()
	require.Contains(t, out, "[usecase-impl-stereotype]",
		"missing annotation must trigger the usecase-impl-stereotype rule")
	require.Contains(t, out, "missing=[RequiredArgsConstructor]",
		"violation message must contain evidence of the missing annotation")
	require.Contains(t, out, "FindUserUseCaseImpl.java",
		"violation must reference the offending source file")
	require.Equal(t, 1, strings.Count(out, "[usecase-impl-stereotype]"),
		"exactly one violation must be reported for a single offending class")
}

// TestAuditCmd_Integration_UsecaseImplStereotype_Determinism runs the missing-
// annotation fixture twice and asserts byte-identical stdout output.
// Backs PC01RNF-003 for the usecase-impl-stereotype rule wiring.
func TestAuditCmd_Integration_UsecaseImplStereotype_Determinism(t *testing.T) {
	t.Parallel()

	// First run.
	workDir1 := t.TempDir()
	copyFixture(t, fixtureDir(t, "pc01us002UsecaseImplStereotype", "projectMissing"), workDir1)
	manifestPath1 := filepath.Join(workDir1, "project-state.yaml")
	stdout1, _, run1 := newAuditCmdForUsecaseImplStereotype(t, workDir1, manifestPath1)
	require.NoError(t, run1("--dir", workDir1, "--manifest", manifestPath1))

	// Second run (separate temp dir — no shared state).
	workDir2 := t.TempDir()
	copyFixture(t, fixtureDir(t, "pc01us002UsecaseImplStereotype", "projectMissing"), workDir2)
	manifestPath2 := filepath.Join(workDir2, "project-state.yaml")
	stdout2, _, run2 := newAuditCmdForUsecaseImplStereotype(t, workDir2, manifestPath2)
